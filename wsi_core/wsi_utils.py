...
def DrawMapFromCoords(
    ...
    downsamples = int(wsi_object.wsi.level_downsamples[vis_level])
    if indices is None:
        indices = np.arange(len(coords))
    ...
    # patch_size = tuple(
    #     np.ceil(
    #         (np.array(patch_size)/np.array(downsamples))
    #     ).astype(np.int32)
    # )
    
    print('downscaled patch size: {}x{}'.format(
        patch_size[0], patch_size[1]
    ))
    scale = downsamples
    patch = np.array(
        wsi_object.wsi.read_region(
            (0, 0),
            0,
            wsi_object.image_size
        ).convert("RGB")
    )
    # patch = cv2.resize(patch, None, None, 1/scale, 1/scale)
    for idx in range(total):
        # if verbose > 0:
        #     if idx % ten_percent_chunk == 0:
        #         print('progress: {}/{} stitched'.format(idx, total))
        ...
        canvas[coord[1]:coord[1]+patch_size[1], coord[0]:coord[0]+patch_size[0], :3] = \
            patch[coord[1]:coord[1]+patch_size[1], coord[0]:coord[0]+patch_size[0], :3]

        if draw_grid:
            DrawGrid(canvas, coord, patch_size)
    canvas = cv2.resize(canvas, None, None, 1/scale, 1/scale)
    canvas_img = Image.fromarray(canvas)
    return canvas_img
