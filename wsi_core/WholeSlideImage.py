...
class WholeSlideImage(object):
    def __init__(self, path):
        ...
        self.image_size = self.level_dim[0]
    ...
    def _assertLevelDownsamples(self):
        ...
        for downsample, dim in zip(self.wsi.level_downsamples, self.wsi.level_dimensions):
            estimated_downsample = (int(dim_0[0]/float(dim[0])), int(dim_0[1]/float(dim[1])))
            level_downsamples.append(estimated_downsample) if estimated_downsample != (downsample, downsample) else level_downsamples.append((downsample, downsample))
            del estimated_downsample
    ...
    def segmentTissue(
        ...
        scale = int(self.wsi.level_downsamples[seg_level])
        scaled_ref_patch_area = int(ref_patch_size**2 / (scale * scale))
        # downsampled_size: tuple = tuple(int(x / scale[0]) for x in self.wsi.dimensions)
        img = np.array(self.wsi.read_region(
            location=(0,0),
            level=0,
            size=self.level_dim[0]
        ))
        resized_img = cv2.resize(img, None, None, 1/scale, 1/scale)
        
        img_hsv = cv2.cvtColor(resized_img, cv2.COLOR_RGB2HSV)  # Convert to HSV space
        img_med = cv2.medianBlur(img_hsv[:,:,1], mthresh)  # Apply median blurring

        # Thresholding
        if use_otsu:
            _, img_otsu = cv2.threshold(
                img_med, 0, sthresh_up, cv2.THRESH_BINARY+cv2.THRESH_OTSU
            )
        else:
            _, img_otsu = cv2.threshold(
    ...
    def visWSI(self,
        ...
        scale = [1/downsample[0], 1/downsample[1]]
        img = cv2.resize(img, None, None, scale[0], scale[1])
        img = img
    ...
    def process_contours(
        ...
        **kwargs
        ...
        save_path_hdf5 = save_path / f"{str(self.name)}.h5"
        print("Creating patches for: ", self.name, "...",)
        elapsed = time.time()
        ...
        for idx, cont in enumerate(self.contours_tissue):
            ...
            asset_dict, attr_dict = self.process_contour(
                cont=cont,
                contour_holes=self.holes_tissue[idx],
    ...
    def process_contour(
        ...
        results = pool.starmap(
            WholeSlideImage.process_coord_candidate,
            iterable
        )
        pool.close()
